...
@onready var animation_player:AnimatedSprite2D = get_node(_animation_player)

@export (NodePath) var _right_run_particles
@onready var right_run_particles:GPUParticles2D = get_node(_right_run_particles)

@export (NodePath) var _left_run_particles
@onready var left_run_particles:GPUParticles2D = get_node(_left_run_particles)
...
func handle_movement_with_gravity():
	...
	player.velocity = player.velocity	# Actually apply all the forces
